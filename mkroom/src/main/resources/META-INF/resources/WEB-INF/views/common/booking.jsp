...
function insertPaymentData(payment) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            productName: payment.name.split("_")[0],
            branchLoaction: payment.name.split("_")[1],
            selectTime: new Date(payment.name.split("_")[2]),
        ...
        success: function(response) {
            ...
            window.location.href = "${pageContext.request.contextPath}/bookingcompl?impUid=" + payment.impUid;
        },
        error: function(xhr, status) {
        }
    })
...
function sendEmailAfterPayment(payment) {
    $.ajax({
        ...
        data: JSON.stringify({
            userId: $("#userId").val(),
            questName: payment.name.split("_")[0],
            branchLocation: payment.name.split("_")[1],
            selectTime: payment.name.split("_")[2].substring(0, 18),
            amount: payment.amount,
            teamMember: $("#teamMember").val(),
        }),
        contentType: "application/json",
